...
bool ArticleDatabase::add_to_database(hash_t hash, Article article) {
	return _database.insert(database_container::value_type(hash, std::move(article))).second;
}
...
void ArticleDatabase::load_from_headers() {
	...
	std::pair<database_container::iterator, bool> new_article;
	for (auto&& path : _paths_to_headers) { //iterate through all header files
		...
		while (std::getline(file, line)) {
			...
			if (!in_header && line.find(BEGIN_HEADER_) != my_string::npos) {
				in_header = true;
				new_article = _database.insert(database_container::value_type(counter++, Article()));
			}
			...
			if (in_header) {
				if ((index = line.find(HEADER_CATEGORIES_)) != my_string::npos) {
					...
					for (auto&& cat : cats) {
						new_article.first->second.categories.insert(StringHelpers::Trim(cat));
					}
				}
				else if ((index = line.find(HEADER_AUTHOR_NAME_)) != my_string::npos) {
					new_article.first->second.author_name = StringHelpers::Trim(line.substr(index + sizeof(HEADER_AUTHOR_NAME_)));
				}
				else if ((index = line.find(HEADER_AUTHOR_ID_)) != my_string::npos) {
					new_article.first->second.author_id = std::stoi(StringHelpers::Trim(line.substr(index + sizeof(HEADER_AUTHOR_NAME_))));
				}
				else if ((index = line.find(HEADER_MAIN_HASH_)) != my_string::npos) {
					new_article.first->second.main_hash = std::stol(StringHelpers::Trim(line.substr(index + sizeof(HEADER_MAIN_HASH_))));
				}
				else if ((index = line.find(HEADER_FILE_PATH_)) != my_string::npos) {
					new_article.first->second.path_to_article_file = StringHelpers::Trim(line.substr(index + sizeof(HEADER_FILE_PATH_)));
				}
			}
			else if (in_hash) {
				...
				new_article.first->second.hashes.insert(hashes_container::value_type(which_position, HashWrapper(h, pl)));
			}
		}
	}
